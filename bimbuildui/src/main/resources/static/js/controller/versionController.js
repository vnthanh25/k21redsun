...
define(['require', 'angular'], function (require, angular) {
	app.aController(bimbuildui.prefix + 'versionController', function($scope, $state, $rootScope, $mdDialog, $http, $log, $window, $location, $filter, $translate, $translatePartialLoader, versionService) {
		if(typeof(bimbuildui.translate.version) === 'undefined' || bimbuildui.translate.version.indexOf($translate.use()) < 0) {
			console.log(bimbuildui.translate.version);
			if(typeof(bimbuildui.translate.version) === 'undefined') {
				bimbuildui.translate.version = '';
			}
			bimbuildui.translate.version += $translate.use() + ';';
			$translatePartialLoader.addPart(bimbuildui.contextPath + '/js/common/message/version');
			$translate.refresh();
		}
		
		var unRegister = $rootScope.$on('$translateChangeSuccess', function () {
	    	console.log('bimbuildui_version_title');
		    $scope.title = $translate.instant('bimbuildui_version_title');
		});
		// Unregister
		...
		$scope.initForm = function(id) {
			$scope.createNew();
			$scope.version.id = id;
			if($scope.version.id > -1) {
				$scope.getById($scope.version.id);
			}
	    ...
	    $scope.showDialog = function () {
	        var htmlUrlTemplate = bimbuildui.contextPath + '/view/version_form.html';
		...
		$scope.save = function() {
			...
			var result;
			if($scope.version.id > -1) {
				result = versionService.update($scope.version.id, $scope.version);
			} else {
				result = versionService.create($scope.version);
			}
			result
			// success.
		...
		$scope.listWithCriteriasByPage = function(pageNo) {
			...
			versionService.listWithCriteriasByPage($scope.getSearch(), pageNo - 1, $scope.page.pageSize, $scope.getSort())
			// success.
			.then(function(response) {
				if(response.status === bimbuildui.httpStatus.code.OK) {
